...
@can('course_academic_session_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.course-academic-sessions.create') }}">
                        ...
                        <tr data-entry-id="{{ $courseAcademicSession->id }}">
                            ...
                            <td>
                                @can('course_academic_session_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.course-academic-sessions.show', $courseAcademicSession->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('course_academic_session_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.course-academic-sessions.edit', $courseAcademicSession->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('course_academic_session_delete')
                                    <form action="{{ route('admin.course-academic-sessions.destroy', $courseAcademicSession->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('course_academic_session_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.course-academic-sessions.massDestroy') }}",
