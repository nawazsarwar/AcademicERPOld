...
@can('hall_student_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.hall-students.create') }}">
                        ...
                        <tr data-entry-id="{{ $hallStudent->id }}">
                            ...
                            <td>
                                @can('hall_student_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.hall-students.show', $hallStudent->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('hall_student_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.hall-students.edit', $hallStudent->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('hall_student_delete')
                                    <form action="{{ route('admin.hall-students.destroy', $hallStudent->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('hall_student_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.hall-students.massDestroy') }}",
