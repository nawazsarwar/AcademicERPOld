...
@can('course_paper_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.course-papers.create') }}">
                        ...
                        <tr data-entry-id="{{ $coursePaper->id }}">
                            ...
                            <td>
                                @can('course_paper_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.course-papers.show', $coursePaper->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('course_paper_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.course-papers.edit', $coursePaper->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('course_paper_delete')
                                    <form action="{{ route('admin.course-papers.destroy', $coursePaper->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('course_paper_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.course-papers.massDestroy') }}",
