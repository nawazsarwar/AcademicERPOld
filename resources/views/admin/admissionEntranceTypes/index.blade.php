...
@can('admission_entrance_type_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.admission-entrance-types.create') }}">
                        ...
                        <tr data-entry-id="{{ $admissionEntranceType->id }}">
                            ...
                            <td>
                                @can('admission_entrance_type_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.admission-entrance-types.show', $admissionEntranceType->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('admission_entrance_type_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.admission-entrance-types.edit', $admissionEntranceType->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('admission_entrance_type_delete')
                                    <form action="{{ route('admin.admission-entrance-types.destroy', $admissionEntranceType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('admission_entrance_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.admission-entrance-types.massDestroy') }}",
