    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-campusHalls">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        @foreach($halls as $key => $hall)
                            <tr data-entry-id="{{ $hall->id }}">
                                ...
                                <td>
                                    @can('hall_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.halls.show', $hall->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('hall_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.halls.edit', $hall->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('hall_delete')
                                        <form action="{{ route('admin.halls.destroy', $hall->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-campusHalls:not(.ajaxTable)').DataTable({ buttons: dtButtons })
