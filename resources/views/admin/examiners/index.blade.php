...
@can('examiner_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.examiners.create') }}">
                ...
                <tbody>
                    @foreach($examiners as $key => $examiner)
                        <tr data-entry-id="{{ $examiner->id }}">
                            ...
                            <td>
                                @can('examiner_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.examiners.show', $examiner->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('examiner_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.examiners.edit', $examiner->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('examiner_delete')
                                    <form action="{{ route('admin.examiners.destroy', $examiner->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('examiner_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.examiners.massDestroy') }}",
