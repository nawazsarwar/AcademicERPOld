    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.dialogues.update", [$dialogue->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('merchant') ? 'is-invalid' : '' }}" name="merchant_id" id="merchant_id" required>
                    @foreach($merchants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('merchant_id') ? old('merchant_id') : $dialogue->merchant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('transaction') ? 'is-invalid' : '' }}" name="transaction_id" id="transaction_id">
                    @foreach($transactions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('transaction_id') ? old('transaction_id') : $dialogue->transaction->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pingback_url') ? 'is-invalid' : '' }}" type="text" name="pingback_url" id="pingback_url" value="{{ old('pingback_url', $dialogue->pingback_url) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('request_type') ? 'is-invalid' : '' }}" type="text" name="request_type" id="request_type" value="{{ old('request_type', $dialogue->request_type) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('raw_response') ? 'is-invalid' : '' }}" name="raw_response" id="raw_response">{{ old('raw_response', $dialogue->raw_response) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('response') ? 'is-invalid' : '' }}" name="response" id="response">{{ old('response', $dialogue->response) }}</textarea>
