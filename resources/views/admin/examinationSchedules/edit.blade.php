...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.examinationSchedule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.examination-schedules.update", [$examinationSchedule->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('academic_session') ? 'is-invalid' : '' }}" name="academic_session_id" id="academic_session_id" required>
                    @foreach($academic_sessions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('academic_session_id') ? old('academic_session_id') : $examinationSchedule->academic_session->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('paper') ? 'is-invalid' : '' }}" name="paper_id" id="paper_id" required>
                    @foreach($papers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('paper_id') ? old('paper_id') : $examinationSchedule->paper->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('paper'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('mode') ? 'is-invalid' : '' }}" name="mode" id="mode" required>
                    ...
                    @foreach(App\Models\ExaminationSchedule::MODE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('mode', $examinationSchedule->mode) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('mode'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('centre') ? 'is-invalid' : '' }}" type="text" name="centre" id="centre" value="{{ old('centre', $examinationSchedule->centre) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start') ? 'is-invalid' : '' }}" type="text" name="start" id="start" value="{{ old('start', $examinationSchedule->start) }}" required>
                @if($errors->has('start'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('end') ? 'is-invalid' : '' }}" type="text" name="end" id="end" value="{{ old('end', $examinationSchedule->end) }}" required>
                @if($errors->has('end'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('booklet_pages') ? 'is-invalid' : '' }}" type="number" name="booklet_pages" id="booklet_pages" value="{{ old('booklet_pages', $examinationSchedule->booklet_pages) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('season') ? 'is-invalid' : '' }}" type="text" name="season" id="season" value="{{ old('season', $examinationSchedule->season) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" name="remarks" id="remarks">{{ old('remarks', $examinationSchedule->remarks) }}</textarea>
