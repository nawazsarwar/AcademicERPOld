...
@can('examination_schedule_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.examination-schedules.create') }}">
                        ...
                        <tr data-entry-id="{{ $examinationSchedule->id }}">
                            ...
                            <td>
                                @can('examination_schedule_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.examination-schedules.show', $examinationSchedule->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('examination_schedule_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.examination-schedules.edit', $examinationSchedule->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('examination_schedule_delete')
                                    <form action="{{ route('admin.examination-schedules.destroy', $examinationSchedule->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('examination_schedule_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.examination-schedules.massDestroy') }}",
