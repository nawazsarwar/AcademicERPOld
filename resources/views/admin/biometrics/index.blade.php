...
@can('biometric_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.biometrics.create') }}">
                ...
                <tbody>
                    @foreach($biometrics as $key => $biometric)
                        <tr data-entry-id="{{ $biometric->id }}">
                            ...
                            <td>
                                @can('biometric_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.biometrics.show', $biometric->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('biometric_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.biometrics.edit', $biometric->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('biometric_delete')
                                    <form action="{{ route('admin.biometrics.destroy', $biometric->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('biometric_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.biometrics.massDestroy') }}",
