...
@can('programme_delivery_mode_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.programme-delivery-modes.create') }}">
                        ...
                        <tr data-entry-id="{{ $programmeDeliveryMode->id }}">
                            ...
                            <td>
                                @can('programme_delivery_mode_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.programme-delivery-modes.show', $programmeDeliveryMode->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('programme_delivery_mode_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.programme-delivery-modes.edit', $programmeDeliveryMode->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('programme_delivery_mode_delete')
                                    <form action="{{ route('admin.programme-delivery-modes.destroy', $programmeDeliveryMode->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('programme_delivery_mode_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.programme-delivery-modes.massDestroy') }}",
