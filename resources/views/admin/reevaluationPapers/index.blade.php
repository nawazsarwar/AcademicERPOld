...
@can('reevaluation_paper_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.reevaluation-papers.create') }}">
                        ...
                        <tr data-entry-id="{{ $reevaluationPaper->id }}">
                            ...
                            <td>
                                @can('reevaluation_paper_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.reevaluation-papers.show', $reevaluationPaper->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('reevaluation_paper_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.reevaluation-papers.edit', $reevaluationPaper->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('reevaluation_paper_delete')
                                    <form action="{{ route('admin.reevaluation-papers.destroy', $reevaluationPaper->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('reevaluation_paper_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.reevaluation-papers.massDestroy') }}",
