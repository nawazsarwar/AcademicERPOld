...
@can('designation_type_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.designation-types.create') }}">
                        ...
                        <tr data-entry-id="{{ $designationType->id }}">
                            ...
                            <td>
                                @can('designation_type_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.designation-types.show', $designationType->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('designation_type_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.designation-types.edit', $designationType->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('designation_type_delete')
                                    <form action="{{ route('admin.designation-types.destroy', $designationType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('designation_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.designation-types.massDestroy') }}",
