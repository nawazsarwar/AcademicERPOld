...
@can('clients_merchant_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.clients-merchants.create') }}">
                        ...
                        <tr data-entry-id="{{ $clientsMerchant->id }}">
                            ...
                            <td>
                                @can('clients_merchant_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.clients-merchants.show', $clientsMerchant->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('clients_merchant_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.clients-merchants.edit', $clientsMerchant->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('clients_merchant_delete')
                                    <form action="{{ route('admin.clients-merchants.destroy', $clientsMerchant->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('clients_merchant_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.clients-merchants.massDestroy') }}",
