    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.course-students.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('session_admitted') ? 'is-invalid' : '' }}" name="session_admitted_id" id="session_admitted_id" required>
                    @foreach($session_admitteds as $id => $entry)
                        <option value="{{ $id }}" {{ old('session_admitted_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('session_admitted'))
                    <span class="text-danger">{{ $errors->first('session_admitted') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('admitted_on') ? 'is-invalid' : '' }}" type="text" name="admitted_on" id="admitted_on" value="{{ old('admitted_on') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration_extension') ? 'is-invalid' : '' }}" type="number" name="duration_extension" id="duration_extension" value="{{ old('duration_extension', '') }}" step="1">
