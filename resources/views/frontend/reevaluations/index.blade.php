        ...
        <div class="col-md-12">
            @can('reevaluation_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.reevaluations.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $reevaluation->id }}">
                                        ...
                                        <td>
                                            @can('reevaluation_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.reevaluations.show', $reevaluation->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('reevaluation_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.reevaluations.edit', $reevaluation->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('reevaluation_delete')
                                                <form action="{{ route('frontend.reevaluations.destroy', $reevaluation->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('reevaluation_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.reevaluations.massDestroy') }}",
