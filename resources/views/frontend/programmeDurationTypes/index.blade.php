        ...
        <div class="col-md-12">
            @can('programme_duration_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.programme-duration-types.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $programmeDurationType->id }}">
                                        ...
                                        <td>
                                            @can('programme_duration_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.programme-duration-types.show', $programmeDurationType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('programme_duration_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.programme-duration-types.edit', $programmeDurationType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('programme_duration_type_delete')
                                                <form action="{{ route('frontend.programme-duration-types.destroy', $programmeDurationType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('programme_duration_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.programme-duration-types.massDestroy') }}",
