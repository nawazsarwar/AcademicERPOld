        ...
        <div class="col-md-12">
            @can('notificable_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.notificables.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $notificable->id }}">
                                        ...
                                        <td>
                                            @can('notificable_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.notificables.show', $notificable->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('notificable_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.notificables.edit', $notificable->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('notificable_delete')
                                                <form action="{{ route('frontend.notificables.destroy', $notificable->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('notificable_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.notificables.massDestroy') }}",
